...
def build_player_display_map(df: pd.DataFrame, player_col: str = "player_id") -> dict:
    ...
    Returns dict: {internal_id: "Player 01", ...}
    """
...
def add_player_display_column(df: pd.DataFrame, player_display_map: dict) -> pd.DataFrame:
    """
    Add player_display column using the mapping.
    """
    df = df.copy()
...
def add_event_display_column(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "date" in df.columns:
        if np.issubdtype(df["date"].dtype, np.datetime64):
            date_str = df["date"].dt.strftime("%m-%d-%Y")
        else:
            date_str = pd.to_datetime(df["date"]).dt.strftime("%m-%d-%Y")
